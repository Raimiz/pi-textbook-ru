...
第一次尝试时不要看最终实现。卡住时，让陪练只检查当前 Lab 的输入、输出和第一处
偏差；不要让它一次写完 Store。
:::
...
|---|---|---|---|
| `ready` | 打开完整文件 | 读取完整记录 | 进入 FIFO |
| `needs-repair` | 打开时发现无换行尾部 | 返回完整前缀和 warning | 拒绝，不调用 I/O |
| `tainted` | 本实例一次底层 append 失败 | 返回最初的 I/O 错误 | 返回同一错误，不调用 I/O |
