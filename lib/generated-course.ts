...
export const chapters: Chapter[] = [
  ...
  },
  {
    ...
    "html": "<h2 id=\"你将得到什么\"><a class=\"heading-anchor\" href=\"#你将得到什么\" aria-label=\"链接到 你将得到什么\">#</a>你将得到什么</h2><p>上一章的事件已经有可靠形状，却仍像一张一次性数组。真实模型会在未知时间逐块返回文本；界面需要立刻显示 delta，Agent loop 又需要等待最终消息。进入本章时，你只能遍历静态事件；完成后将得到 <code>workshop/src/event-stream.ts</code>：它既可被 <code>for await...of</code> 逐项消费，也可通过 <code>result()</code> 等待最终值。</p>\n<p>本章只增加一种复杂性：<strong>时间</strong>。不定义消息语义，也不连接网络。你会观察生产者快于消费者、消费者先等待、终态到达以及取消四条路径。</p>\n<p>本章不变量是：</p>\n<blockquote>\n<p>每条流必须以一个可识别的终态结束；迭代器和 <code>result()</code> 必须从同一个终态得到一致结论，不能有一方永远等待。</p>\n</blockquote>\n<p>要恢复起点，撤销 <code>workshop/src/event-stream.ts</code> 和对应测试中本章故障实验的改动，再运行聚焦测试。不要用 <code>setTimeout</code> 延长测试来掩盖悬挂。</p>\n<h2 id=\"先建立全景\"><a class=\"heading-anchor\" href=\"#先建立全景\" aria-label=\"链接到 先建立全景\">#</a>先建立全景</h2><p><code>AsyncIterable&lt;T&gt;</code> 只回答“下一项怎样异步到达”，没有方便的最终结果通道。普通消费者如果只拼 delta，会错过 stop reason、usage 和错误信息。我们需要把两个视角放在同一个对象中：</p>\n<figure class=\"code-frame\"><figcaption><span>text</span><button type=\"button\" data-copy-code aria-label=\"复制 text 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-text\">生产者 push(e1) ─────┐\n         push(e2) ───┼──→ for await (过程证据)\n         push(done) ─┘\n              │\n              └─────────→ result() (最终事实)</code></pre></figure><p>事件流不是回调列表。消费者每次请求下一项；若队列已有事件就立即取出，否则登记一个 waiter。终态事件既要交给迭代器，也要解析为最终结果。</p>\n<section class=\"learning-block learning-block--predict\" aria-label=\"运行前先判断\"><header><span class=\"block-kicker\">先预测</span><h4>运行前先判断</h4><small>先写判断，再看推理</small></header><div class=\"block-body prediction-question\"><p>消费者收到第一个 delta 后 <code>break</code>，生产者是否会自动停止？另一个调用了 <code>result()</code> 的任务会自动完成吗？</p>\n</div><details class=\"prediction-answer\"><summary>展开参考推理</summary><div><p>都不会因为 <code>break</code> 自动取消。退出迭代只表示这个消费者不再拉取；生产者必须收到并观察 <code>AbortSignal</code> 才会停止。若生产者继续推送最终事件，<code>result()</code> 仍会完成；若生产者既不停也不发终态，它就会悬挂。</p>\n</div></details></section><h2 id=\"一条流需要队列等待者与终态\"><a class=\"heading-anchor\" href=\"#一条流需要队列等待者与终态\" aria-label=\"链接到 一条流需要队列、等待者与终态\">#</a>一条流需要队列、等待者与终态</h2><p>下面是核心数据结构。构造器接收两个纯函数：怎样识别终态，以及怎样从终态提取结果。这样通用容器不需要理解模型消息：</p>\n<figure class=\"code-frame\"><figcaption><span>ts</span><button type=\"button\" data-copy-code aria-label=\"复制 ts 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-ts\">export class EventStream&lt;T, R = T&gt; implements AsyncIterable&lt;T&gt; {\n  private queue: T[] = [];\n  private waiting: Array&lt;(item: IteratorResult&lt;T&gt;) =&gt; void&gt; = [];\n  private done = false;\n  private readonly finalResult: Promise&lt;R&gt;;\n  private resolveFinalResult!: (result: R) =&gt; void;\n\n  constructor(\n    private readonly isComplete: (event: T) =&gt; boolean,\n    private readonly extractResult: (event: T) =&gt; R,\n  ) {\n    this.finalResult = new Promise&lt;R&gt;((resolve) =&gt; {\n      this.resolveFinalResult = resolve;\n    });\n  }\n\n  result(): Promise&lt;R&gt; {\n    return this.finalResult;\n  }\n}</code></pre></figure><p><code>queue</code> 保存“事件先到”的情况，<code>waiting</code> 保存“消费者先等”的情况。<code>done</code> 防止终态后继续写入。<code>finalResult</code> 在构造时创建一次，所以先调用还是后调用 <code>result()</code> 都指向同一事实。</p>\n<section class=\"learning-block learning-block--mechanism\" aria-label=\"为什么不是两个互不相关的 Promise\"><header><span class=\"block-kicker\">关键机制</span><h4>为什么不是两个互不相关的 Promise</h4><small>把现象连接到不变量</small></header><div class=\"block-body\"><p>如果迭代结束和最终结果由不同代码路径控制，就会出现 iterator 已结束但 result 仍 pending，或 result 已完成却继续收到 delta。终态事件必须是唯一提交点；容器在同一次 <code>push</code> 中完成两边。</p>\n</div></section><p>这一版容器明确只支持一个事件消费者。多个地方都调用 <code>result()</code> 没问题，因为它们等待同一个 Promise；但两个 <code>for await</code> 会争抢同一队列，而不是各自看到完整广播。这个限制应当写进契约，而不是碰巧隐藏在实现里。界面与 Agent loop 如果都需要过程事件，后续应由上层建立明确的转发机制；贸然把 <code>queue</code> 复制成多份，会同时引入订阅取消、慢消费者占用内存和终态回收问题。</p>\n<p>所谓背压也要准确理解：AsyncIterable 让消费者决定何时请求下一项，却不能自动阻止一个主动 <code>push()</code> 的远端生产者。若 provider 比 UI 快，队列仍会增长。课程先用模型响应这种有限流量建立正确语义；面对无界日志或字节流时，还需要容量上限、暂停策略或丢弃策略。不要把语法上的 <code>for await</code> 误当作完整资源治理。</p>\n<p><code>push()</code> 的关键不是数组操作，而是顺序：先识别终态并解析结果，再把该事件交给等待者或队列。终态本身仍然可观察。</p>\n<figure class=\"code-frame\"><figcaption><span>ts</span><button type=\"button\" data-copy-code aria-label=\"复制 ts 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-ts\">push(event: T): void {\n  if (this.done) return;\n\n  if (this.isComplete(event)) {\n    this.done = true;\n    this.resolveFinalResult(this.extractResult(event));\n  }\n\n  const waiter = this.waiting.shift();\n  if (waiter) waiter({ value: event, done: false });\n  else this.queue.push(event);\n}</code></pre></figure><p>异步迭代器循环遵守同样的优先级：先清空队列，再看是否结束，最后才等待新事件。这可避免终态已入队却被 <code>done</code> 提前截掉。</p>\n<section class=\"learning-block learning-block--lab\" aria-label=\"实践 2.1 · 同时证明过程和结果\"><header><span class=\"block-kicker\">动手实现</span><h4>实践 2.1 · 同时证明过程和结果</h4><small>在真实文件中建立能力</small></header><div class=\"block-body\"><p><strong>目标：</strong> 让一个终态完成两种消费方式。</p>\n<p><strong>文件：</strong> <code>workshop/src/event-stream.ts</code>、<code>workshop/test/model-stream.test.ts</code></p>\n<p><strong>动作：</strong></p>\n<ol>\n<li>补齐 <code>push()</code> 和异步迭代器。</li>\n<li>先推送两个 delta 和一个 done，再开始迭代，覆盖队列路径。</li>\n<li>另写测试先启动迭代再推送，覆盖 waiter 路径。</li>\n<li>同时断言事件序列与 <code>await stream.result()</code>。</li>\n</ol>\n<p><strong>运行：</strong> <code>npm run workshop:test -- event-stream</code></p>\n<p><strong>预期：</strong> 两种时序都得到 <code>delta, delta, done</code>；最终结果相同，测试没有计时依赖。</p>\n</div></section><h2 id=\"取消必须沿调用链协作\"><a class=\"heading-anchor\" href=\"#取消必须沿调用链协作\" aria-label=\"链接到 取消必须沿调用链协作\">#</a>取消必须沿调用链协作</h2><p>JavaScript 不能安全强杀任意 Promise。<code>AbortController</code> 只广播意图，等待函数、provider 和工具必须主动响应。一个可取消等待应同时处理“进入前已取消”和“等待中取消”：</p>\n<figure class=\"code-frame\"><figcaption><span>ts</span><button type=\"button\" data-copy-code aria-label=\"复制 ts 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-ts\">export function abortableDelay(ms: number, signal?: AbortSignal): Promise&lt;void&gt; {\n  if (signal?.aborted) {\n    return Promise.reject(new DOMException(&quot;aborted&quot;, &quot;AbortError&quot;));\n  }\n  return new Promise((resolve, reject) =&gt; {\n    const timer = setTimeout(resolve, ms);\n    signal?.addEventListener(&quot;abort&quot;, () =&gt; {\n      clearTimeout(timer);\n      reject(new DOMException(&quot;aborted&quot;, &quot;AbortError&quot;));\n    }, { once: true });\n  });\n}</code></pre></figure><p>底层等待可以抛 <code>AbortError</code>，但流的公共边界不能让它变成一个无人处理的 rejected promise。生产者捕获后应推送协议规定的 <code>error</code> 终态；下一章会让该事件携带 <code>stopReason: &quot;aborted&quot;</code> 的最终 canonical assistant message。</p>\n<section class=\"learning-block learning-block--lab\" aria-label=\"实践 2.2 · 让取消也完成 result\"><header><span class=\"block-kicker\">动手实现</span><h4>实践 2.2 · 让取消也完成 result</h4><small>在真实文件中建立能力</small></header><div class=\"block-body\"><p><strong>目标：</strong> 验证取消不是“停止打印”，而是一个完整终态。</p>\n<p><strong>文件：</strong> <code>workshop/test/model-stream.test.ts</code></p>\n<p><strong>动作：</strong></p>\n<ol>\n<li>创建带延迟的生产者，先推一个 delta。</li>\n<li>消费该 delta 后调用 <code>controller.abort()</code>。</li>\n<li>由生产者捕获取消并推送 <code>error</code> 终态。</li>\n<li>同时断言迭代器结束、<code>result()</code> 完成且没有后续 delta。</li>\n</ol>\n<p><strong>运行：</strong> <code>npm run workshop:test -- event-stream</code></p>\n<p><strong>预期：</strong> 事件模式为 <code>delta → error(aborted)</code>；测试在有限时间内自然结束。</p>\n</div></section><aside class=\"learning-block learning-block--pi\" aria-label=\"与当前上游 Pi 对照\"><header><span class=\"block-kicker\">Pi 源码对照</span><h4>与当前上游 Pi 对照</h4><small>课程模型与生产实现</small></header><div class=\"block-body\"><p>固定提交 <code>8479bd8</code> 的 <code>packages/ai/src/utils/event-stream.ts</code> 同样让 <code>EventStream&lt;T,R&gt;</code> 实现 <code>AsyncIterable&lt;T&gt;</code> 并提供 <code>result(): Promise&lt;R&gt;</code>；<code>AssistantMessageEventStream</code> 把 <code>done</code> 和 <code>error</code> 都识别为终态。课程暂时使用更小事件集并限制单消费者。上游模型错误与取消也进入流内终态，而不是随机从调用点向外抛出。</p>\n</div></aside><h2 id=\"故意把它弄坏\"><a class=\"heading-anchor\" href=\"#故意把它弄坏\" aria-label=\"链接到 故意把它弄坏\">#</a>故意把它弄坏</h2><p>最隐蔽的错误是只通知迭代器结束，却忘记完成最终 Promise：</p>\n<figure class=\"code-frame\"><figcaption><span>ts</span><button type=\"button\" data-copy-code aria-label=\"复制 ts 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-ts\">// 错误示例\nend(): void {\n  this.done = true;\n  while (this.waiting.length &gt; 0) {\n    this.waiting.shift()?.({ value: undefined, done: true });\n  }\n  // result() 永远 pending\n}</code></pre></figure><section class=\"learning-block learning-block--failure\" aria-label=\"预期失败 · 制造一个悬挂 result\"><header><span class=\"block-kicker\">故障实验</span><h4>预期失败 · 制造一个悬挂 result</h4><small>寻找第一次偏差</small></header><div class=\"block-body\"><p>临时绕过终态事件，直接把 <code>done</code> 设为 <code>true</code>。用 <code>Promise.race</code> 加一个很短但宽松的测试超时，首次偏差应明确为“result 未完成”。不要通过增加超时修复；恢复唯一终态提交点，使 result 与 iterator 同步结束。</p>\n</div></section><h2 id=\"本章验收\"><a class=\"heading-anchor\" href=\"#本章验收\" aria-label=\"链接到 本章验收\">#</a>本章验收</h2><section class=\"learning-block learning-block--checkpoint\" aria-label=\"Checkpoint 02 · 时间成为显式协议\"><header><span class=\"block-kicker\">本章关卡</span><h4>Checkpoint 02 · 时间成为显式协议</h4><small>以证据进入下一状态</small></header><div class=\"block-body\"><p>运行 <code>npm run workshop:test -- event-stream</code>，队列路径、waiter 路径、正常终态和取消路径都应通过。你能解释为什么终态必须被迭代到、为什么 <code>break</code> 不等于 abort，以及为何 result 不能从 delta 临时拼出。恢复时只撤销故障实验。下一章将把通用 <code>T</code> 和 <code>R</code> 换成 Agent 的消息语言。</p>\n</div></section><h2 id=\"可选迁移练习\"><a class=\"heading-anchor\" href=\"#可选迁移练习\" aria-label=\"链接到 可选迁移练习\">#</a>可选迁移练习</h2><section class=\"learning-block learning-block--transfer\" aria-label=\"迁移 · 构造可取消的字节流\"><header><span class=\"block-kicker\">可选迁移</span><h4>迁移 · 构造可取消的字节流</h4><small>完成引导重建后再减少脚手架</small></header><div class=\"block-body\"><p>不给 starter：定义 <code>ByteEvent = chunk | done | error</code>，用 <code>EventStream&lt;ByteEvent, Uint8Array&gt;</code> 传送三段字节。要求启动前取消和中途取消都产生终态，最终字节顺序保持不变。测试不能只断言长度，还要断言事件顺序和 <code>result()</code>。</p>\n</div></section><h2 id=\"小结\"><a class=\"heading-anchor\" href=\"#小结\" aria-label=\"链接到 小结\">#</a>小结</h2><p>事件流把时间从隐藏的回调行为变成了显式协议。AsyncIterable 提供过程证据，<code>result()</code> 提供最终事实，唯一终态把两者锁在一起；AbortSignal 则把取消意图沿调用链传递。下一章不再使用无意义的示例字符串，而会定义 Pi 内部长期依赖的 canonical message IR。</p>\n",
    "toc": [
      {
        "id": "你将得到什么",
        "title": "你将得到什么",
  ...
  },
  {
    ...
    "html": "<h2 id=\"你将得到什么\"><a class=\"heading-anchor\" href=\"#你将得到什么\" aria-label=\"链接到 你将得到什么\">#</a>你将得到什么</h2><p>我们已有消息语言和事件流，却没有事件生产者。直接连接真实 LLM 会同时引入网络、鉴权、费用、限流和随机措辞；测试失败时，你无法判断是协议错了，还是远端恰好换了回答。</p>\n<p>本章只增加一种主要复杂性：<strong>一个遵守真实模型边界的确定性生产者</strong>。你会实现 <code>Model</code> 接口与 <code>ScriptedModel</code>，按轮消费预先声明的脚本，记录每次收到的 context，并覆盖成功、运行错误和取消。</p>\n<p>本章不变量是：</p>\n<blockquote>\n<p>ScriptedModel 与真实 adapter 必须满足同一个 <code>Model.stream()</code> 契约；调用者不应根据“是不是 fake”改变消费代码。</p>\n</blockquote>\n<p>完成后修改 <code>workshop/src/types.ts</code>、<code>workshop/src/scripted-model.ts</code> 及测试。恢复起点时重新使用测试中的原始 turns，撤销故意插入的失败回合；不需要删除缓存或配置 API key。</p>\n<h2 id=\"先建立全景\"><a class=\"heading-anchor\" href=\"#先建立全景\" aria-label=\"链接到 先建立全景\">#</a>先建立全景</h2><p>一个随便返回 <code>&quot;hello&quot;</code> 的 mock 只能帮助测试 UI。我们需要的是可执行规格：脚本描述过程，模型把过程翻译成 canonical events，<code>result()</code> 给出最终 <code>AssistantMessage</code>。</p>\n<figure class=\"code-frame\"><figcaption><span>text</span><button type=\"button\" data-copy-code aria-label=\"复制 text 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-text\">AgentContext\n    │\n    ▼\nScriptedModel.stream(context, { signal })\n    ├─ 立即返回 EventStream\n    ├─ 保存 context 快照\n    └─ 在 microtask 中消费一个 ScriptedTurn\n          TextContent → text_delta\n          ToolCall → toolcall_delta + toolcall_end\n          成功 AssistantMessage → done\n          error / aborted AssistantMessage → error</code></pre></figure><p>“立即返回流”很重要：消费者可以先订阅，再等待事件。模型内部的认证、运行和网络类失败不能随机从 <code>stream()</code> 调用点抛出，而要成为流的最终 <code>error</code> 事件。</p>\n<section class=\"learning-block learning-block--predict\" aria-label=\"运行前先判断\"><header><span class=\"block-kicker\">先预测</span><h4>运行前先判断</h4><small>先写判断，再看推理</small></header><div class=\"block-body prediction-question\"><p>如果 ScriptedModel 只记录传入的 <code>context</code> 引用，Agent 随后向 <code>context.messages</code> 追加消息，<code>requests[0]</code> 会代表调用发生时的输入，还是修改后的输入？</p>\n</div><details class=\"prediction-answer\"><summary>展开参考推理</summary><div><p>它会随原数组一起变化，失去历史证据。ScriptedModel 应在调用时保存快照；测试才能证明第二轮请求究竟看到了哪些消息，而不是读取事后被修改的对象。</p>\n</div></details></section><h2 id=\"先固定-model-的唯一入口\"><a class=\"heading-anchor\" href=\"#先固定-model-的唯一入口\" aria-label=\"链接到 先固定 Model 的唯一入口\">#</a>先固定 Model 的唯一入口</h2><p>上层只依赖一个小接口。<code>Model</code> 不暴露供应商 SDK 类型，也不返回完整数组：</p>\n<figure class=\"code-frame\"><figcaption><span>ts</span><button type=\"button\" data-copy-code aria-label=\"复制 ts 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-ts\">export interface Model {\n  stream(\n    context: AgentContext,\n    options?: { signal?: AbortSignal },\n  ): ModelStream;\n}\n\nexport interface ModelStream extends AsyncIterable&lt;ModelEvent&gt; {\n  result(): Promise&lt;AssistantMessage&gt;;\n}\n\nexport type ScriptedTurn =\n  | AssistantMessage\n  | {\n      stopReason: &quot;error&quot; | &quot;aborted&quot;;\n      errorMessage: string;\n      partialText?: string;\n    };</code></pre></figure><p>一个 <code>ScriptedTurn</code> 描述本回合最终要形成的 canonical message，而不是硬编码 <code>ModelEvent[]</code>。ScriptedModel 仍需像真实 adapter 一样先发 start，再把 content block 投影成 delta/end，最后依据 stop reason 产生 done 或 error。测试因此验证的是边界行为，不是把预期事件原样返还。</p>\n<p>Turn 是最终事实，不是界面输出；同一回合仍可被终端、JSON 事件消费者或未来的 Agent loop 以不同方式观察。这一点很关键。</p>\n<section class=\"learning-block learning-block--mechanism\" aria-label=\"Fake 的价值是控制变量\"><header><span class=\"block-kicker\">关键机制</span><h4>Fake 的价值是控制变量</h4><small>把现象连接到不变量</small></header><div class=\"block-body\"><p>当 Agent loop 测试需要“先请求 read，再回答完成”时，脚本精确固定两轮模型行为。系统中唯一变化的是 loop 或 tool；失败就能定位到被测试层。真实模型则用于证明兼容性，不能替代确定性规格。</p>\n</div></section><section class=\"learning-block learning-block--lab\" aria-label=\"实践 4.1 · 播放一个文本回合\"><header><span class=\"block-kicker\">动手实现</span><h4>实践 4.1 · 播放一个文本回合</h4><small>在真实文件中建立能力</small></header><div class=\"block-body\"><p><strong>目标：</strong> 让同一份脚本同时产生过程事件和最终消息。</p>\n<p><strong>文件：</strong> <code>workshop/src/types.ts</code>、<code>workshop/src/scripted-model.ts</code></p>\n<p><strong>动作：</strong></p>\n<ol>\n<li>定义 <code>ModelStream</code>、<code>Model</code> 与 <code>ScriptedTurn</code>。</li>\n<li>用含两个 text block、<code>stopReason: &quot;stop&quot;</code> 的 AssistantMessage 构造模型。</li>\n<li>收集 <code>for await</code> 事件，同时等待 <code>stream.result()</code>。</li>\n<li>断言最终 content 与 scripted turn 一致。</li>\n</ol>\n<p><strong>运行：</strong> <code>npm run workshop:test -- scripted-model</code></p>\n<p><strong>预期：</strong> 事件模式为 <code>start → text_delta → text_delta → done</code>，最终文本稳定为脚本内容。</p>\n</div></section><h2 id=\"按轮消费并保存请求快照\"><a class=\"heading-anchor\" href=\"#按轮消费并保存请求快照\" aria-label=\"链接到 按轮消费并保存请求快照\">#</a>按轮消费并保存请求快照</h2><p>Agent 会多次调用同一个 Model。ScriptedModel 因此接收 <code>ScriptedTurn[]</code>，用 cursor 每次只消费一个回合。回合内部的 content block 顺序就是事件投影顺序。</p>\n<figure class=\"code-frame\"><figcaption><span>ts</span><button type=\"button\" data-copy-code aria-label=\"复制 ts 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-ts\">const model = new ScriptedModel([\n  assistantMessage([\n    text(&quot;我先读取文件。&quot;),\n    { type: &quot;toolCall&quot;, id: &quot;c1&quot;, name: &quot;read&quot;,\n      arguments: { path: &quot;README.md&quot; } },\n  ], &quot;toolUse&quot;),\n  assistantMessage([\n    text(&quot;项目用于学习 Agent。&quot;),\n  ], &quot;stop&quot;),\n]);</code></pre></figure><p>第一次 <code>stream()</code> 的结果必须包含 tool call 且 stop reason 为 <code>toolUse</code>；第二次则是普通文本终止。<code>requests</code> 保存 <code>structuredClone(context)</code>，于是测试能检查第二轮 context 是否包含第一轮 assistant message 和配对 tool result。第 07 章会用这段脚本闭合 Agent loop。</p>\n<p>ScriptedModel 的入口骨架应保持同步返回：</p>\n<figure class=\"code-frame\"><figcaption><span>ts</span><button type=\"button\" data-copy-code aria-label=\"复制 ts 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-ts\">stream(\n  context: AgentContext,\n  options: { signal?: AbortSignal } = {},\n) {\n  const stream = new AssistantMessageEventStream();\n  this.requests.push(structuredClone(context));\n  const turn = this.turns[this.cursor++];\n  queueMicrotask(() =&gt; {\n    if (options.signal?.aborted) {\n      const aborted = assistantMessage([], &quot;aborted&quot;, {\n        errorMessage: &quot;Request was aborted&quot;,\n      });\n      stream.push({ type: &quot;error&quot;, reason: &quot;aborted&quot;, error: aborted });\n      stream.end(aborted);\n      return;\n    }\n    // 完成态源码随后投影 content blocks，并 push done 或 error\n  });\n  return stream;\n}</code></pre></figure><p>若回合耗尽，microtask 应产生 <code>stopReason: &quot;error&quot;</code> 的最终 message 和 <code>error</code> 事件，而不是让 <code>stream()</code> throw。这样“测试配置错误”也会验证公共失败契约。</p>\n<p>播放过程还需要维护一个 partial message。每个 content block 先形成对应事件，再把已走过的 content slice 放入 partial；终态只提交原 scripted turn，不能重新从事件文本猜一次结果。这样事件观察者和 <code>result()</code> 对每个 block 的归属一致。为避免测试代码意外修改结果，模型在消费 turn 时使用 <code>structuredClone</code>。</p>\n<p>课程 ScriptedModel 刻意没有 delay step。它验证协议和回合顺序，不用操作系统时间证明业务顺序；取消测试覆盖“播放前 signal 已 abort”。真实 transport 的中途取消留给下一章。如果未来加入延迟，也必须使用可控调度器，不能靠 5ms 阈值猜测机器负载。</p>\n<p>请求快照也不等于生产日志。它只服务测试断言，不能保存密钥或无限累积到长期进程；真实产品的可观测性会由独立事件层负责。保持这个限制，ScriptedModel 才不会悄悄长成第二套 session。</p>\n<section class=\"learning-block learning-block--lab\" aria-label=\"实践 4.2 · 证明第二轮输入\"><header><span class=\"block-kicker\">动手实现</span><h4>实践 4.2 · 证明第二轮输入</h4><small>在真实文件中建立能力</small></header><div class=\"block-body\"><p><strong>目标：</strong> 把 ScriptedModel 变成 Agent 状态机的观测探针。</p>\n<p><strong>文件：</strong> <code>workshop/test/model-stream.test.ts</code></p>\n<p><strong>动作：</strong></p>\n<ol>\n<li>使用上面的两轮脚本。</li>\n<li>调用第一轮后，构造含 assistant 与 toolResult 的新 context。</li>\n<li>调用第二轮，再修改原 context 数组。</li>\n<li>断言 <code>model.requests[1]</code> 仍是调用时快照。</li>\n</ol>\n<p><strong>运行：</strong> <code>npm run workshop:test -- scripted-model</code></p>\n<p><strong>预期：</strong> 两轮结果分别为 <code>toolUse</code> 与 <code>stop</code>；请求快照不受事后修改影响。</p>\n</div></section><h2 id=\"错误和取消也是脚本能力\"><a class=\"heading-anchor\" href=\"#错误和取消也是脚本能力\" aria-label=\"链接到 错误和取消也是脚本能力\">#</a>错误和取消也是脚本能力</h2><p>成功路径不足以定义模型边界。错误回合可直接声明 <code>partialText</code>；启动前已经 abort 的 signal 则产生 aborted 终态，不播放原回合。前者的稳定 trace 是：</p>\n<figure class=\"code-frame\"><figcaption><span>text</span><button type=\"button\" data-copy-code aria-label=\"复制 text 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-text\">start\ntext_delta &quot;正在&quot;\nerror reason=error\nresult.stopReason = &quot;error&quot;\nresult.content = [{ type: &quot;text&quot;, text: &quot;正在&quot; }]</code></pre></figure><p><code>result()</code> 在这里 resolve，而不是 reject；调用者得到一条 canonical assistant message，可同时读取部分文本和 <code>errorMessage</code>。这与“把错误吞掉并伪造成 stop”完全不同。</p>\n<aside class=\"learning-block learning-block--pi\" aria-label=\"与当前上游 Pi 对照\"><header><span class=\"block-kicker\">Pi 源码对照</span><h4>与当前上游 Pi 对照</h4><small>课程模型与生产实现</small></header><div class=\"block-body\"><p>固定提交 <code>8479bd8</code> 的 <code>packages/ai/src/providers/faux.ts</code> 提供更丰富的确定性 provider，可生成内容、usage、错误与取消事件。课程版只保留按轮脚本和请求快照，命名也不追求同构；两者共同点是遵守与真实 provider 相同的流协议，而不是返回测试专用捷径。</p>\n</div></aside><h2 id=\"故意把它弄坏\"><a class=\"heading-anchor\" href=\"#故意把它弄坏\" aria-label=\"链接到 故意把它弄坏\">#</a>故意把它弄坏</h2><p>最常见的假模型会在失败时直接抛出：</p>\n<figure class=\"code-frame\"><figcaption><span>ts</span><button type=\"button\" data-copy-code aria-label=\"复制 ts 代码\">复制</button></figcaption><pre tabindex=\"0\"><code class=\"language-ts\">stream(): ModelStream {\n  throw new Error(&quot;script exhausted&quot;); // 错误：调用者甚至拿不到流\n}</code></pre></figure><p>这迫使上层同时处理同步 throw、rejected promise 和流内 error，破坏统一边界。</p>\n<section class=\"learning-block learning-block--failure\" aria-label=\"预期失败 · 让 script exhausted 向外抛\"><header><span class=\"block-kicker\">故障实验</span><h4>预期失败 · 让 script exhausted 向外抛</h4><small>寻找第一次偏差</small></header><div class=\"block-body\"><p>临时把脚本耗尽路径改为同步 throw。契约测试的首次偏差应是 <code>stream()</code> 没有返回 <code>EventStream</code>。恢复为立即返回流，并在微任务中推送 error 终态；再断言 <code>await result()</code> 得到 <code>stopReason: &quot;error&quot;</code>。</p>\n</div></section><h2 id=\"本章验收\"><a class=\"heading-anchor\" href=\"#本章验收\" aria-label=\"链接到 本章验收\">#</a>本章验收</h2><section class=\"learning-block learning-block--checkpoint\" aria-label=\"Checkpoint 04 · 模型成为可执行规格\"><header><span class=\"block-kicker\">本章关卡</span><h4>Checkpoint 04 · 模型成为可执行规格</h4><small>以证据进入下一状态</small></header><div class=\"block-body\"><p>运行 <code>npm run workshop:test -- scripted-model</code>，文本 blocks、工具调用、两轮快照、错误 turn、脚本耗尽和启动前取消测试应全部通过。你应能解释为什么 fake 必须遵守生产接口、为什么保存快照，以及为何错误通过流终态表达。下一章只替换 turn 来源：把真实 OpenAI-compatible chunk 翻译成相同事件。</p>\n</div></section><h2 id=\"可选迁移练习\"><a class=\"heading-anchor\" href=\"#可选迁移练习\" aria-label=\"链接到 可选迁移练习\">#</a>可选迁移练习</h2><section class=\"learning-block learning-block--transfer\" aria-label=\"迁移 · 部分输出后限流失败\"><header><span class=\"block-kicker\">可选迁移</span><h4>迁移 · 部分输出后限流失败</h4><small>完成引导重建后再减少脚手架</small></header><div class=\"block-body\"><p>独立加入 <code>{ stopReason: &quot;error&quot;, errorMessage: &quot;rate limited&quot;, partialText: &quot;正在&quot; }</code> 回合。断言 <code>start → text_delta → error</code>、partial content、<code>errorMessage</code> 和 result stop reason。不要给 ScriptedModel 增加 <code>isFake</code> 分支，也不要让测试读取 cursor。</p>\n</div></section><h2 id=\"小结\"><a class=\"heading-anchor\" href=\"#小结\" aria-label=\"链接到 小结\">#</a>小结</h2><p>ScriptedModel 不是低配模型，而是协议的确定性实现。它让请求快照、content block 投影、tool call、终态、错误与启动前取消都可被精确复现。现在 Agent 上层已有稳定测试基座；下一章接入真实 provider 时，只需证明翻译边界等价，而不重新发明模型接口。</p>\n",
    "toc": [
      {
        "id": "你将得到什么",
        "title": "你将得到什么",
